...
## 분기 보정 백로그 (판정 데이터 축적 후 평가 — 미구현)
가중치·임계값 변경은 분기마다 published 전망 전수의 적중률 데이터로 평가하고 채택 시 MODEL_VERSION을 올린다.
- **[B1] momentum 최신값 가중** — A(모멘텀) 채점 시 최신 변화율 부호가 trend_3p 방향과 **반대**일 때 A점수에 **±0.5 보정**(반대 부호 쪽으로 끌어내림). 근거: 부산-LA(2026-06) 최신 −6.5% 하락인데 월 변화율 [−6.5%, +8.4%, +54.6%] 중 2개 상승이라 trend_3p=up_2 → momentum +1. 변동성 큰 월간 KITA 데이터에서 추세-최신 괴리가 판정을 과대평가할 수 있음. **평가 기준**: 채택 전후 적중률(특히 월간 항로) 비교, 표본 충분(≥분기) 시점에 결정.
