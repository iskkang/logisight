...
**2026-08 결정: 도메인은 `logisight.mtlship.com` 유지.** 따라서 분리는 인프라가 아니라
표기 층위에서 한다 — 독자 대면 산출물(이메일 푸터·리포트 표지·발행 정보)의 MTL 문구는
Logisight 단독 표기로 정리 완료. 소유 공시는 사이트 소개 페이지가 담당한다(사이트 레포).

`NEWSLETTER_FROM` · `SITE_URL` 환경변수는 휴면 훅으로 남는다 — 훗날 도메인을 옮기게 되면
DNS·Resend 발신 도메인 검증 후 값만 바꾸면 된다(미설정 시 현행 값 유지, 동작 불변).
