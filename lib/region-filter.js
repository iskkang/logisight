...
const SHARED_TYPE_RULES = [
  '항목은 briefType별로 다룬다.',
  'news = 해당 권역의 운영 동향.',
  'policy = 정책/거시 변화와 그 함의(해당 권역 중심).',
  'trade = 무역량/물동량 동향. 기사에 보고된 수치만 문맥상 인용하고 표는 만들지 않는다.',
  '한국은 기사가 실제로 한국과 연관(한국 기업·한국 교역·한국발/향 화물)될 때만 언급한다. 연관이 없으면 한국을 아예 언급하지 않는다 — 해당 권역 자체의 사안으로만 서술하고, "한국 연관 없음"·"보고되지 않았다" 같은 문장이나 "한국 화주 시사점" 결론도 쓰지 않는다.',
