...
const { normalizeMarkdownBody, sanitizeImageUrl } = require('../generators/web/lib/news-pipeline');
 ...
 * @param {string} [opts.generatedBy]    - 우리 AI 가 정리한 글이면 'Logisight AI'.
 *                                       모델명을 넣지 않는다 —— 독자에게 필요한 건 책임 주체지
 *                                       어느 모델을 썼는지가 아니다. 사람이 쓴 글이면 넘기지 않는다.
 * @param {string} [opts.defaultCategory]
 * @param {string} [opts.forcedCategory] - 지정 시 frontmatter·classifyCategory 무시
 */
async function insertArticle({
  ...
  const imageUrl = sanitizeImageUrl(extractFm(markdownContent, 'image_url'));
  const imageSource = extractFm(markdownContent, 'image_source');
  const imageCredit = extractFm(markdownContent, 'image_credit');
  const tags     = extractTags(markdownContent);

  // maritime_news category 값은 해상|항공|철도|물류|무역으로 제한
  const VALID_CATEGORIES = ['해상', '항공', '철도', '물류', '무역'];
  const normalizedCategory = forcedCategory
    || (() => {
      const raw = extractFm(markdownContent, 'category') || defaultCategory
        || classifyCategory(title ?? '', subtitle ?? '');
      return VALID_CATEGORIES.includes(raw) ? raw : classifyCategory(title ?? '', subtitle ?? '');
    })();
  ...
  const row = {
    ...
    source: sourceName || 'Logisight',
    lang: 'ko',
    category: normalizedCategory,
    agent_type: agentType,
    tags: tags.length ? tags : null,
    slug: extractSlug(canonicalUrl),
    content: normalizeMarkdownBody(extractContent(markdownContent), {
      title,
      summary: subtitle,
      imageUrl,
      imageCredit,
  ...
  // 표기는 지우는 게 아니라 붙이는 것으로 한다. 값이 없으면 열을 만들지 않는다 —
  // 빈 문자열을 넣으면 "AI가 썼는데 모델을 모른다"와 "사람이 썼다"가 같아진다.
  if (generatedBy) row.generated_by = generatedBy;
  if (publishedAt) row.published_at = publishedAt;
  if (imageUrl) row.image_url = imageUrl;
  if (imageSource) row.image_source = imageSource;
  if (imageCredit) row.image_credit = imageCredit;
