...
// 가드는 언어별로 나눈다. 한국어 어휘로 일본어 초안을 검사하면 전부 통과해버려
// 가드가 있으나 마나가 된다 — 「必ず」는 '반드시' 목록에 걸리지 않는다.
const GUARD = {
  ko: {
    forbidden: ['확실', '반드시', '틀림없', '분명히', '할 것이다'],
    hedge: /(추정|가량|약\s|예상|범위|내외|정도|~|∼|〜|가능성|로 보인다|수 있다)/,
    // 정량(지연일·풍속·비용 등)은 범위·추정 동반 필수. 좌표·거리(°·km)는 입력 근거라 제외.
    qty: /(\d+(?:\.\d+)?)\s*(일|시간|%|달러|USD|원|TEU|FEU|m\/s|kt|노트|hPa)/,
    move: '통과|도달|상륙|접근|관통|북상|남하|이동',
    unit: '일|시간',
    after: '후|뒤|내',
  },
  ja: {
    forbidden: ['確実', '必ず', '間違いな', '明らかに', 'するだろう', 'に違いな'],
    hedge: /(推定|程度|前後|見込|範囲|およそ|約\s?\d|可能性|~|∼|〜|とみられる|しうる|おそれ)/,
    qty: /(\d+(?:\.\d+)?)\s*(日|時間|%|ドル|USD|円|TEU|FEU|m\/s|kt|ノット|hPa)/,
    move: '通過|到達|上陸|接近|貫通|北上|南下|移動',
    unit: '日|時間',
    after: '後|以内|以降',
  },
};

for (const g of Object.values(GUARD)) {
  // 트랙 끝점 너머 시점 단언: 이동동사 ↔ 일/시간 수치 인접(트랙 점별 예보시각 없음).
  g.trackTime = new RegExp(
    `(${g.move})[^.。\\n]{0,15}\\d+\\s*(${g.unit})|\\d+\\s*(${g.unit})\\s*(${g.after})[^.。\\n]{0,15}(${g.move})`,
  );
}

const FORBIDDEN = GUARD.ko.forbidden; // 기존 호출부 호환

// 언어별로 달라지는 것은 출력 언어 지시와 금지 표현의 예시뿐이다.
// 사실·근거·분량 규칙은 언어와 무관하므로 한 벌만 유지한다.
const LANG = {
  ko: {
    write: '한국어로',
    tone: '[표현 규칙] 확률·추정 표현을 쓰고, 인과를 단정하지 마라(정합/추정/상관 수준). "확실/반드시/틀림없/분명히/~할 것이다"류 단정 금지.',
  },
  ja: {
    write: '日本語で(常体・だ/である)',
...
/**
 * @param {object} ctx
 * @param {'ko'|'ja'} [lang='ko'] 출력 언어. validateClimate도 같은 값을 받아야 한다.
 */
function buildClimatePrompt(ctx, lang = 'ko') {
  const L = LANG[lang] || LANG.ko;
  const { event, route, viaPassage, viaMinKm, sharedRoutes = [], trackSummary = {}, nearbyAssets = [] } = ctx;
  // 관문명은 프롬프트에도 들어간다. 한국어로 넣으면 일본어 본문에 '미야코해협'이 섞인다.
  const viaName = (lang === 'ja' ? viaPassage.name_ja || viaPassage.name_ko : viaPassage.name_ko);
  ...
  const system = [
    `당신은 글로벌 해상물류 리스크 분석가다. 아래 "실데이터"만 사용해 ${L.write} ${isSeismic ? '재해' : '기후'} 영향 초안을 쓴다. 입력(event_route_impacts·track)에 없는 노선·항만·수치·사실을 만들지 않는다.`,
    ...
    L.tone,
    // event_echo는 대조용 키다. 번역하면 echo 검사가 전부 실패한다.
    '출력은 JSON 하나: {"weather":"...","impact":"...","action":"...","event_echo":"<이벤트명 그대로>"}. event_echo는 주어진 이벤트명을 번역하지 말고 그대로 반복.',
  ].join('\n');
  ...
  const facts = {
    ...
    예보트랙: ts.hasTrack
      ...
      : '없음(점 이벤트) — 단일좌표 기준',
    걸린_관문: { 관문: viaName, 트랙_최소거리_km: Math.round(viaMinKm) },
...
/**
 * @param {object} ctx  ctx.lang 이 'ja'면 일본어 가드로 검사한다(기본 'ko').
 */
function validateClimate(parsed, ctx) {
  const g = GUARD[ctx.lang] || GUARD.ko;
  const issues = [];
  ...
  const all = `${weather}\n${impact}\n${action}`;
  if (g.forbidden.some((w) => all.includes(w))) issues.push('단정 표현 포함');
  ...
  for (const [label, txt] of [['weather', weather], ['impact', impact]]) {
    if (g.qty.test(txt) && !g.hedge.test(txt)) { issues.push(`정량 단정(${label}) — 추정·범위 표현 누락`); break; }
  }
  // 2c) 트랙 끝점 너머 시점 단언 차단(트랙 점별 예보시각 미제공).
  if (g.trackTime.test(`${weather}\n${impact}`)) issues.push('트랙 시각 미제공인데 통과 시점 단언');
...
async function narrateClimate(callLLM, ctx, { maxRetries = 1, lang = 'ko' } = {}) {
  const prompt = buildClimatePrompt(ctx, lang);
  let last = { issues: ['미실행'] };
...
module.exports = { buildClimatePrompt, validateClimate, narrateClimate, safeParse, FORBIDDEN, GUARD, LANG };
