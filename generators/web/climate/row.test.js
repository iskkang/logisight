...
const { mapClimateRow, EDITOR_PLACEHOLDER, isCritical } = require('./row');
...
test('isCritical: severity=r → CRITICAL, a → 아님', () => {
  assert.equal(isCritical({ severity: 'r' }), true);
  assert.equal(isCritical({ severity: 'a' }), false);
});

test('mapClimateRow: CRITICAL(태풍 red)는 가드 통과해도 자동 보류(draft)', () => {
  ...
  assert.ok(r.data_quality_flags.includes('hold:critical'));
  assert.ok(r.data_quality_flags.some((f) => /^gate:/.test(f)));
});

test('mapClimateRow: WARNING(a) + 가드 통과 → 자동발행(published)', () => {
...
test('mapClimateRow: 가드 실패(needs_editor) → 보류(draft) + hold:guard_fail, placeholder', () => {
  const r = mapClimateRow(warning, { needs_editor: true }, asof);
  assert.equal(r.status, 'draft');
...
test('mapClimateRow: 귀속 불명(via/route 누락) → 보류 + hold:no_attribution', () => {
  const noAttr = { ...warning, viaPassage: { id: '', name_ko: '' } };
  const r = mapClimateRow(noAttr, prose, asof);
  assert.equal(r.status, 'draft');
  assert.ok(r.data_quality_flags.includes('hold:no_attribution'));
});

test('mapClimateRow: 연결 키 metric_ref = climate:<route>:<event>:<via>', () => {
  assert.equal(mapClimateRow(critical, prose, asof).metric_ref, 'climate:r3:hko-2611:miyako_strait');
});

test('mapClimateRow: statement 3단, impact_note 권장행동(발행분)', () => {
  const r = mapClimateRow(warning, prose, asof);
  assert.match(r.statement, /기상 리스크 변화/);
  assert.ok(r.impact_note.includes(prose.action));
});

test('mapClimateRow: basis에 걸린 관문·트랙 교차 명시', () => {
  const b = mapClimateRow(critical, prose, asof).basis;
  assert.ok(b.some((x) => x.includes('미야코해협') && /트랙|교차/.test(x)));
});

test('mapClimateRow: confidence_reason에 발행/보류 사유', () => {
  assert.match(mapClimateRow(warning, prose, asof).confidence_reason, /자동발행/);
  assert.match(mapClimateRow(critical, prose, asof).confidence_reason, /보류|critical/);
});
