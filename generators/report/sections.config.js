...
const SECTIONS = [
  ...
  // ■ "실무 시황 리포트체"를 평서체로 읽는 사고 ★
  // 2026-08(9건)·2026-09(7건) 두 달 연속으로 맺음말이 formal-ending 에 걸려 발행이
  // 막혔다. 2026-07 이 0건이었던 것은 모델이 잘 써서가 아니라 사람이 손으로 고쳤기
  // 때문이다(ed94d41 "사용자 편집 v2 반영") —— 이 프롬프트로 생성만 한 달은 전부 걸렸다.
  //
  // 원인은 프롬프트 안의 충돌이다. 맨 앞에서 "§13-8 실무 시황 리포트체 전면 적용"이라
  // 크게 말하고, 명사형 종결은 열다섯 줄 뒤 맨 끝에 한 번 나온다. "리포트체"라는 말과
  // "이는 ~을 의미/시사" 같은 예시는 그 자체로 평서체를 부른다. 모델은 앞에서 크게
  // 말한 쪽을 따랐고, 실제로 "시사한다·국면이다·뚜렷했다·판단된다"로 끝냈다.
  // 그래서 종결 규칙을 문체를 소개하는 바로 그 자리로 끌어올리고, 걸린 어미를 그대로
  // 변환 예시로 박아 둔다. 맨 끝의 규칙도 남겨 둔다(두 번 말해서 손해 볼 것이 없다).
  {
    id: 'closing',
    title: '07. 이달의 프레임 — 맺음말',
    focus:
      '리포트 전체를 닫는 총평. ★문체는 스타일 가이드 §13-8(실무 시황 리포트체) 전면 적용. ' +
      '단 "리포트체"를 평서체로 오해하지 말 것 — 종결은 §2 명사형이다. ' +
      '("시사한다." ✕ → "~을 시사" ○ / "국면이다." ✕ → "~ 국면" ○ / "뚜렷했다." ✕ → "~ 뚜렷" ○ / "판단된다." ✕ → "~로 판단" ○) ' +
      '짧은 문단(1~3문장), 한 문장 한 정보, 수치 문장과 해석 문장 분리("이는 ~을 의미/시사" — 여기서도 "의미한다"가 아니라 "의미"로 끝낸다), 비유·브랜딩 조어 금지, 산문에서는 ▲▼ 대신 "전월 대비 N% 상승/하락" 표기. 서술형 소제목(###) 1개 + 볼드 리드 문장으로 시작. 표·차트 금지. ' +
      '재료는 두 가지뿐: [본 리포트 각 섹션의 핵심] 다이제스트와 [파생 지표] 수치. ' +
      '★ 재료에 없는 회사명·인명·협상 조건·인프라 사업 등 새 사실·수치 도입은 그럴듯해도 전부 실격(창작 판정) — 모델 자체 지식 사용 금지. ' +
      '★ 인과 서술은 2층위 결합: 파생 지표는 가격 행동(무엇이 움직였나)을, 관련 기사는 메커니즘(선복 증감·GRI/PSS 공표·기관 분석 — 왜 움직였나)을 입증. 메커니즘 주장은 반드시 기사 근거+기관 귀속("Xeneta에 따르면")으로만 쓰고, 기사 근거 없으면 §13-8 9항의 행위 서술로 후퇴. ' +
