...
const SECTIONS = [
  ...
  },
  {
    ...
    focus:
      '리포트 전체를 닫는 총평. ★문체는 스타일 가이드 §13-8(실무 시황 리포트체) 전면 적용 — 짧은 문단(1~3문장), 한 문장 한 정보, 수치 문장과 해석 문장 분리("이는 ~을 의미/시사"), 비유·브랜딩 조어 금지, 산문에서는 ▲▼ 대신 "전월 대비 N% 상승/하락" 표기. 서술형 소제목(###) 1개 + 볼드 리드 문장으로 시작. 표·차트 금지. ' +
      ...
      '구조: 소제목+리드 문장 = 이달을 규정하는 단일 프레임(서술형 명명). ' +
      '본론 = 그 프레임을 파생 지표 2~3개로 입증. ★각 논점 문단에 [파생 지표] 블록의 수치를 반드시 1개 이상 그대로 인용(CCFI/SCFI %, 전이 시차 N주, 공시 갭 %, 탈동조화, 벙커 괴리 등) 후 해석 — 수치 인용 없이 렌즈 용어만 차용하는 것은 실격. "렌즈" 등 방법론 용어 본문 노출 금지. ' +
      '반증 = "반대로 조정 가능성도 존재" 문단으로 프레임에 반하는 신호 1개를 정직하게 제시. ' +
      '후반 = "다음 달 체크 포인트는 세 가지" 후 첫째/둘째/셋째 각각 별도 문단(각 "~여부" + 선행 지표·판정 기준), 마지막은 "결론적으로 ~ 장세/국면" 요약 문단(권고 아님 — 시장 서술만). ' +
