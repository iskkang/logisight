...
// ── 이 파일을 고칠 때의 규칙 ───────────────────────────────────────────
// focus에 **팩트시트에 없는 고유명사·필드명·URL을 쓰지 않는다.**
// 모델은 지시받은 이름을 본문으로 옮기고, 검수는 「팩트시트에 없는 사실」로
// 막는다. 재시도로는 못 빠져나온다 — 지시 자체가 근거 없는 것을 요구하기 때문이다.
// 2026-06호에서 네 번 반복했다:
//   ERAI의 변화율 기준     → 팩트시트에 changeBasis를 넣어 해결
//   벤치마크 도구 URL      → 본문에서 빼고 조립 단계에서 붙이도록 변경
//   「他セクション」 언급   → 통계 이름으로 쓰게 변경
//   IATA・TAC 지수명       → 이름을 빼고 「航空のスポット指数」로만
// 새 지시를 넣을 때는 근거를 팩트시트에 함께 넣거나, 이름을 빼고 쓴다.
// ───────────────────────────────────────────────────────────────────
...
const SECTIONS = [
  {
    ...
    focus:
      ...
      + '【今月の芯】signals にある「為替の効きの差」を必ず扱う。外貨建て契約の系列(外航・国際航空)と'
  ...
  },
  {
    ...
    focus:
      ...
      + '【為替の内訳】前年同月比のうち為替換算による上乗せ分は算出済みで渡している。自分で計算しない。'
      ...
      + '【単位】欠航数・計画数は「便数」である。TEU ではない。'
      + '「58便が欠航」と書く。「58TEU」は誤りである。'
      + '【範囲】scope のとおり主要East-West航路であって日本発着ではない。その範囲を一度明示する。'
      + '【使い方】直近の公表回が入っている。欠航が増えたか減ったかを言い、'
      + 'スポット運賃の方向と並べて述べる。ただし欠航が運賃を動かしたとは書かない — 因果は主張しない。'
      + '(5) 荷動き: route 軸に日本海事センターの航路別コンテナ荷動きがある。最後の小見出しで必ず扱う。'
      + '【なぜ重要か】本レポートで「日本の荷物が実際どれだけ動いたか」を航路単位で示せる唯一の数字である。'
      + '港湾統計は日本の港のTEUであって行き先の航路を示さず、貿易統計は金額であって量ではない。'
      + '【書き方】日本発の数字と、比較対象の国の数字を必ず並べる。'
      + '「日本発米国向けは5.4万TEU(▲3.2%)、一方で中国発は+25.5%」の形で対比する。'
      + '【欧州】europe に日本単独の数字は無い。日本は北東アジアに含まれる — 日本の数字として書かない。'
      + '【禁止】港湾統計のTEUと足したり、一方から他方を説明したりしない。母集団が異なる。',
  },
  {
    id: 'air',
    no: '03',
    ...
    focus:
      ...
      + '航空のスポット指数は本レポートのデータに無い。断るのは一文だけにする — '
  ...
  },
  {
    ...
    focus:
      ...
      + 'periodMismatch が true のとき、総論だけでなく本文でも対象月のずれを断る — 読者は総論を読み飛ばす。'
      + '【書き方】「他のセクション」ではなく統計の名前で書く。どのセクションが何を扱うかは'
      + 'factsheet に無く、書けば根拠のない推測になる(実際に検査で差し戻された)。'
      + 'periods にある軸名で述べる — 例:「企業向けサービス価格指数と貿易統計は2026年6月分だが、'
      + '港湾統計は2026年5月分の速報値である」。'
  ...
  },
  {
    ...
    focus:
      ...
      + '【基準月】港湾統計に触れるなら、それが何月分かを必ず書く。'
      + 'periodMismatch が true で、港湾統計だけ他の統計より1か月古い。'
      + '「6月分速報値に続く」のように暗示だけで済ませない(検査で差し戻された)。'
